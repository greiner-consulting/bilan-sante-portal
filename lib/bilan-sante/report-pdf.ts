...
function wrapSingleParagraph(font: PDFFont, text: string, size: number, maxWidth: number): string[] {
  const paragraph = String(text ?? "").trim();
  if (!paragraph) return [""];

  const rawTokens = paragraph.split(/\s+/g).filter(Boolean);
  const tokens = rawTokens.flatMap((token) => splitLongToken(font, token, size, maxWidth));
  const lines: string[] = [];
  let current = "";
...
function headerFractions(headers: string[]): number[] {
  ...
  if (headers.length === 2) {
    ...
    if (normalized[0].includes("dimension")) return [0.18, 0.34, 0.18, 0.15, 0.15].slice(0, headers.length);
    return [0.36, 0.64];
  }
...
function tableRowHeight(params: {
  ...
  row.forEach((cell, index) => {
    const lines = wrapText(font, safeText(cell), size, Math.max(10, widths[index] - paddingX * 2));
...
function drawSectionTitle(params: {
  ...
  let cursor = ensureSpace({ doc: params.doc, cursor: params.cursor, requiredHeight: 36 });
...
function drawParagraphs(params: {
  ...
  for (const paragraph of params.paragraphs) {
    ...
    if (!text) continue;
    const estimated = textHeight(
      wrapText(style.font, text, style.size, availableWidth()).length,
      style.lineHeight
    ) + 8;
    cursor = ensureSpace({ doc: params.doc, cursor, requiredHeight: estimated });
    const used = drawWrappedText({
      page: cursor.page,
      text,
      x: PAGE.marginLeft,
...
function drawTable(params: {
  ...
  if (params.table.title) {
    ...
    const estimated = textHeight(
      wrapText(titleStyle.font, params.table.title, titleStyle.size, availableWidth()).length,
  ...
  cursor = ensureSpace({ doc: params.doc, cursor, requiredHeight: headerEstimate });
  ...
  const rows = params.table.rows.length > 0 ? params.table.rows : [Array.from({ length: params.table.headers.length }, (_, index) => (index === 0 ? "—" : ""))];
