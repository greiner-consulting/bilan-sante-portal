...
function emptyMaterial(area: DialogueArea): V5AreaMaterial {
  return {
...
function normalizeAreaMaterial(area: DialogueArea, raw: any): V5AreaMaterial {
...
function normalizeState(coverage: unknown): V5State | null {
  ...
  if (!raw || raw.version !== 5) return null;
...
function mergeCoverage(existing: unknown, state: V5State) {
  const base =
    existing && typeof existing === "object" && !Array.isArray(existing)
      ? { ...(existing as Record<string, unknown>) }
      : {};
  return { ...base, [V5_KEY]: state };
}
...
function currentState(row: any): V5State {
  return normalizeState(row?.coverage_json) ?? emptyState();
}

function crossDomainMemory(state: V5State): CrossDomainMemory[] {
...
function toDialogueMaterial(area: DialogueArea, material: V5AreaMaterial): DialogueAreaMaterial {
...
async function handleStructuredIntake(params: {
  ...
  if (!hasMeaningfulStructuredInput(schema, data)) throw new Error("STRUCTURED_INTAKE_EMPTY");
...
async function concludeArea(params: {
  row: any;
  state: V5State;
  userId: string;
  ...
  const area = params.state.area;
  const material = params.state.materials[area];
  material.final_analysis = params.analysis;
  ...
  await saveState({
    ...
    patch: {
      phase: "dimension_questions",
      iteration: maxIterationsForArea(area),
      question_index: 0,
      question_batch_json: [],
    },
  });
...
async function finalizeIteration(params: {
  ...
  const analysis = await analyzeDiagnosticState({
    ...
    material: toDialogueMaterial(area, material),
    crossDomainMemory: crossDomainMemory(params.state),
    stage: `après itération ${params.iteration}/${maxIterationsForArea(area)}`,
  });
  material.analyses[`iteration_${params.iteration}`] = analysis;

  if (area === "context") {
    await concludeArea({ row: params.row, state: params.state, userId: params.userId, analysis });
    return;
  }

  if (params.iteration < 3) {
    const synthesis = formatIntermediateSynthesis({ area, iteration: params.iteration, analysis });
...
async function answerCurrentQuestion(params: {
  ...
  if (!question?.question) throw new Error("V5_ACTIVE_QUESTION_NOT_FOUND");
  ...
  await finalizeIteration({ row: params.row, state: params.state, userId: params.userId, iteration });
}
...
async function handleReview(params: {
  ...
  if (!isAffirmative(params.message)) {
    material.validation_feedback = [...(material.validation_feedback || []), params.message].slice(-6);
...
function questionsPayload(row: any, state: V5State) {
  if (state.stage !== "questions") return [];
  return Array.isArray(row.question_batch_json) ? row.question_batch_json : [];
}

function responsePayload(row: any, state: V5State, history: any[]) {
  ...
  try {
    ...
    if (!normalizeState(row.coverage_json)) {
      ...
      await saveState({
        row,
        state,
        patch: { status: "collected", question_index: 0, question_batch_json: [] },
      });
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const status =
      ...
      message === "DIAGNOSTIC_ALREADY_COMPLETE" ? 409 :
      message === "STRUCTURED_INTAKE_EMPTY" ? 400 : 500;
    return json({ ok: false, error: message }, status);
  }
}
